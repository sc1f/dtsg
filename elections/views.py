...
def races(request, year):
    ...
    races = Race.objects.filter(year=selected_election.year).all().order_by('id').annotate(num_positions=Count('position')).order_by('order')
...
def positions(request, year, race_id):
    ...
    positions_empty = selected_race.position_set.all().filter(candidate__published=False).annotate(num_candidates=Count('candidate')).order_by('order')
